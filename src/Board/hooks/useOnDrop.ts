...
async function onColumnDrop (cards, result, move) {
	...
	const dir = newSort > oldSort ? 1 : -1
		, carry = cards.current[oldSort];
...
export default function useOnDrop (tripId, cards) {
	...
	return async result => {
		if (result.type === 'card') await onCardDrop(cards, result, moveCard);
		else await onColumnDrop(cards, result, moveColumnWithTripId);
	};
}
