...
export default function Text ({
	...
	const [isEditing, setIsEditing] = useState(false)
		...
		, [preview, setPreview] = useState(substituteUrls(defaultValue || ''));
	...
	return (
		<div
			className={cx(css.notes, className, isEditing && css.editing, onWhite && css.onWhite)}
			tabIndex={isEditing ? void 0 : -1}
			onClick={onClick}
		>
			{isEditing || isValueEmpty ? (
				<>
					<div
						className={css.livePreview}
						dangerouslySetInnerHTML={{__html:value.replace(URL_RX, '<u>$1</u>') + ' '}}
					/>
					<textarea
