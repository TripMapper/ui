...
export default function ReadonlyBoard({ trip }) {
    ...
    if (!trip)
        return 'skeleton';
    const onBoardBackgroundClick = e => (e.target === e.currentTarget && activeCards.length > 0)
        && Emit(Signal.HideReadonlyCard, null);
    return (<div className={css.overflow}>
			<div className={cx(css.board, activeCards.length > 0 && css.cardOpen)} style={{ '--cols': days.length }} onClick={onBoardBackgroundClick}>
				{days.map(day => (<div key={day.key} className={css.column}>
						<header className={cx(css.header, hasStartDate && css.hasDate)}>
							{day.label}
						</header>

						<ul className={css.list}>
							{(cards.current[day.index] ?? []).map(card => (<li key={card.id}>
