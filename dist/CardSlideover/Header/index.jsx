...
export const CARD_SLIDEOVER_HEADER_FRAGMENT = gql `
	...
	${IMAGE_FRAGMENT}
`;
export default function Header({ readOnly = false, id, type, coverImage, name, }) {
    ...
    const onChange = useCallback(debounce(async (e) => {
        ...
        if (error)
            Emit(Signal.Notify, t('save_failed', 'Failed to Save'), 'error', t('card_name_save_failed', 'Unable to save card name'));
    }, 500), []);
    const uploadFiles = async (files) => {
        ...
        if (error)
            Emit(Signal.Notify, t('save_failed', 'Failed to Save'), 'error', t('card_cover_upload_failed', 'Unable to upload cover image'));
    };
    const onInput = e => {
    ...
    useEffect(() => {
        if (!ta.current)
            return;
        const target = ta.current;
        target.value = name;
        onInput({ target });
    }, [ta, name]);
    return (<Dropzone El="header" className={cx(css.header, css[type.toLowerCase()], uploading && css.uploading)} accept="image/png,image/jpeg" hoverClassName={css.dragover} onChange={uploadFiles}>
			{coverImage ? <Image {...coverImage.srcset}/> : (<label className={css.placeholder}>
			...
			{readOnly ? (<h2>{name}</h2>) : (<textarea ref={ta} defaultValue={name} onInput={onInput} onChange={onChange} rows={1}/>)}
		</Dropzone>);
}
