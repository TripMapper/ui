...
export default function Column({ day, hasStartDate, cards }) {
    const self = useRef();
    return (<div className={css.column} ref={self}>
			<Header hasStartDate={hasStartDate} label={day.label}/>
            ...
            // }
            return (<div ref={dragProvided.innerRef} {...dragProvided.draggableProps} style={style}>
							<Droppable droppableId={`day_${day.index}`} type="card">
								{provided => (<>
										<div {...dragProvided.dragHandleProps}>
											<ColumnDragHandle />
										</div>
										<ul className={css.list} ref={provided.innerRef} {...provided.droppableProps}>
											{cards.map((card, i) => (<Draggable key={card.id} draggableId={card.id} index={i}>
													{provided => (<li key={card.id} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
															<Card {...card}/>
														</li>)}
												</Draggable>))}
