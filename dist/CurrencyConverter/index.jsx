...
const appendSym = currencyIso => {
    const iso = currencyIso.toUpperCase(), sym = getCurrencySymbol(currencyIso);
...
export default function CurrencyConverter({ defaultFrom = 'EUR', to }) {
    ...
    return (<figure>
			...
			<div className={css.from}>
				<Select name="" options={opts} defaultValue={from} onChange={v => setFrom(v)} inline/>
				<label>
					<span>{getCurrencySymbol(from.value)}</span>
					<input type="text" inputMode="numeric" pattern="[0-9.,]*" defaultValue={1} onInput={onInput}/>
				</label>
			</div>

			<div className={css.to}>
				<span>{to.toUpperCase()}</span>
				<strong>{formatCurrency(convert((isNaN(value) ? 0 : value), from.value.toString(), to, data?.currenciesList ?? []) ?? 1, to).replace(/\.0+$/, '')}</strong>
			</div>
