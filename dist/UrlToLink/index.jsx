...
export default function UrlToLink({ url }) {
    const [title, setTitle] = useState(null), [icon, setIcon] = useState(null);
    let safeUrl = url;
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const { title, icon } = await persist.get(safeUrl);
            ...
            if (json.title) {
                try {
                    await persist.set(safeUrl, json);
                }
                catch (e) { }
    ...
    return (<a href={safeUrl} className={css.link} target="_blank" rel="noopener noreferrer" onClick={onClick} title={title}>
